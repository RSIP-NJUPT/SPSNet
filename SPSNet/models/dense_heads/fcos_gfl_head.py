...
from SPSNet.registry import MODELS
...
class FCOSGFLHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 init_cfg: MultiConfig = dict(
                     ...
                     override=dict(
                         ...
                         bias_prob=0.01)),
                 **kwargs) -> None:
        self.iou_strategy = iou_strategy
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            eps = 1e-7
            if self.iou_strategy is 'iou':
                ious = bbox_overlaps(pos_decoded_bbox_preds, pos_decoded_target_preds, is_aligned=True).clamp(min=eps)
            elif self.iou_strategy is 'giou':
                ious = giou(pos_decoded_bbox_preds, pos_decoded_target_preds,eps=eps)
            elif self.iou_strategy is 'diou':
                ious = diou(pos_decoded_bbox_preds, pos_decoded_target_preds,eps=eps)
            elif self.iou_strategy is 'ciou':
                ious = ciou(pos_decoded_bbox_preds, pos_decoded_target_preds,eps=eps)

            quality_labels[pos_inds] = ious
            one_hot_labels = F.one_hot(flatten_labels)
            C=one_hot_labels.shape[1]
            expanded_tensor = quality_labels.unsqueeze(1)
            quality_labels = expanded_tensor.repeat(1, C)
            soft_weight_labels = one_hot_labels * quality_labels
            soft_weight_labels = soft_weight_labels[:, :C-1]

            loss_cls = self.loss_cls(
                flatten_cls_scores,soft_weight_labels, avg_factor=num_pos)
...
def giou(pred: Tensor, target: Tensor, eps: float = 1e-7) -> Tensor:
...
def ciou(pred: Tensor, target: Tensor, eps: float = 1e-7) -> Tensor:
    ...
    return cious.clamp(min=0.0, max=1.0)